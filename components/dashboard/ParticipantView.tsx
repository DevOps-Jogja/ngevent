...
export default function ParticipantView({ registrations }: ParticipantViewProps) {
    ...
    return (
        <div>
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-6">{t('dashboard.participant.myRegistrations')}</h2>
            ...
            {registrations.length === 0 ? (
                <div className="bg-white dark:bg-dark-card rounded-2xl shadow-sm p-12 text-center border border-gray-100 dark:border-gray-800 animate-fade-in">
                    ...
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">
                        {t('dashboard.participant.noRegistrations')}
                    </h3>
                    <p className="text-gray-500 dark:text-gray-400 mb-6 max-w-sm mx-auto">
                        {t('dashboard.participant.noRegistrationsDesc')}
                    </p>
                    <Link
                        href="/events"
                    ...
                    >
                        {t('dashboard.participant.browseEvents')}
                    </Link>
                </div>
            ) : (
